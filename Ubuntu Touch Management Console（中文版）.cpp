...
inline void adb_shell(char *cmd) {
	char shell[200] = "adb shell ";
	system(strcat(shell, cmd));
}

inline void cd(char *root) {
	char cmd[200] = "cd ";
...
inline void flash_img(char *code, char *part) {
...
inline void create_lib(char *id, char *name) {
...
inline void libertine(char *id, char *cmd) {
...
inline void inst_pkg(char *id, char *pkg_name) {
...
inline void uninst_pkg(char *id, char *pkg_name) {
...
inline void apk_inst(char *filepath) {
	char inst[500] = "adb install ";
	adb_shell(strcat(inst, filepath));
}

inline void mkdir(char *folder, char *backtype) {
	char dir[500] = "mkdir ";
	strcat(dir, folder);
	strcat(dir, "\\UTBackup-");
	strcat(dir, backtype);
